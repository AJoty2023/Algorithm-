...
void Union(int a, int b){
    ...
    if(a != b){  // If they are not in the same set
        if(sz[a] < sz[b]){
            swap(a, b);  // Swap a and b so that a's set is larger
        }
...
int main(){
    ...
    int total_cost = 0;  // Initialize the total cost of the minimum spanning tree

    // Iterate through each edge in sorted order
    for(auto &edge : edges){
        int wt = edge.first;           // Weight of the edge
    ...
    cout << total_cost << endl;  // Print the total cost of the minimum spanning tree
